...
void *sub1(void *t)
{
  ...
  */
  ...
  /* Użycie pętli while do sprawdzenia warunku */
  while (count < THRESHOLD) {
    pthread_cond_wait(&count_condvar, &count_mutex);
    printf("sub1: thread=%ld Condition variable signal received. count=%d\n", tid, count);
  }
  ...
  for (i = 0; i < ITERATIONS; i++) {
    ...
    */
    if (count == THRESHOLD) {
      ...
      pthread_cond_broadcast(&count_condvar);  // Użycie broadcast zamiast signal
